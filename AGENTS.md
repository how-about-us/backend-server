...
spec 파일(`docs/specs/`)을 기반으로 작업할 때는 아래 규칙을 따른다.

| 시점 | 액션 |
|------|------|
| 작업 항목 완료 직후 | 해당 spec의 체크박스를 `[x]`로 업데이트한다 |
| 구현 완료, PR 올리기 전 | spec 상태 → `진행중`, `_index.md` 목록 항목도 동일하게 반영한다 |
| PR merged + DoD 항목 검증 완료 | spec 상태 → `완료`, `_index.md`에서 체크박스 `[x]` 처리 후 "완료된 Spec" 섹션으로 이동한다 |

- 체크박스 업데이트와 상태 전환은 **별개의 시점**이다. 구현이 끝났어도 DoD 미충족이면 `완료`로 바꾸지 않는다.
- `_index.md`는 spec 상태가 바뀔 때마다 **항상 함께** 갱신한다.
