...
Google Maps에서 가져온 장소 마스터 데이터. 여러 방에서 재사용 가능. (Google API 데이터의 캐싱 역할)
...
| location | GEOGRAPHY(POINT,4326) | NOT NULL | PostGIS 공간 데이터 |
| category | VARCHAR(50) | NULLABLE | 카테고리 (식당/카페/관광지/숙소 등) |
| rating | DECIMAL(2,1) | NULLABLE | 평점 |
| photo_reference | VARCHAR(500) | NULLABLE | Google 사진 참조 키 |
| last_synced_at | TIMESTAMP | NULLABLE | Google API와 마지막 동기화 시점 |
| created_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | 생성일시 |
| updated_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | 수정일시 |

**인덱스:** GIST(location) — 공간 쿼리용
...
2. **places 테이블 분리:** Google Place ID 기준 중복 없이 관리. 여러 방에서 같은 장소 재사용 가능.
3. **message.id auto-increment:** WebSocket 재접속 시 마지막 수신 ID 기반 미수신 메시지 조회 패턴에 최적화.
4. **room.id UUID:** 초대 URL에 노출되므로 추측 불가능한 UUID 사용.
5. **PostGIS location 컬럼:** 경로 추천, 근처 장소 탐색 등 공간 쿼리를 위해 GEOGRAPHY 타입 사용.
...
- [ ] places 테이블 캐시 만료 정책 (Google 데이터 갱신 주기)
