...
| `[x]` | 장소 상세 조회 | 장소명, 주소, 장소 유형 표시명(`primaryTypeDisplayName`), 평점(`rating`), 평점 수(`userRatingCount`), 전화번호, 웹사이트, 요일별 영업시간(`weekdayDescriptions`)을 포함한 전체 영업시간(`regularOpeningHours`), 사진 목록(`photoNames`), 리뷰 목록(`reviews`) 등, 상세 조회 결과는 Redis에 5분 TTL 캐시 | Redis |
