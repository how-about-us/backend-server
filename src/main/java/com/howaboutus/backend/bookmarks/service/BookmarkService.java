...
public class BookmarkService {
    ...
    public BookmarkResult create(UUID roomId, BookmarkCreateCommand command, Long userId) {
        ...
        try {
            BookmarkResult result = BookmarkResult.from(bookmarkRepository.saveAndFlush(bookmark));
            publishChanged(roomId, userId, RoomBookmarkEventType.BOOKMARK_CREATED, result.bookmarkId(),
                    result.categoryId());
            return result;
    ...
    public void delete(UUID roomId, long bookmarkId, Long userId) {
        ...
        Long categoryId = bookmark.getCategory().getId();
        bookmarkRepository.delete(bookmark);
        publishChanged(roomId, userId, RoomBookmarkEventType.BOOKMARK_DELETED, bookmarkId, categoryId);
    }
    ...
    public BookmarkResult updateCategory(UUID roomId, long bookmarkId, long categoryId, Long userId) {
        ...
        BookmarkResult result = BookmarkResult.from(bookmarkRepository.saveAndFlush(bookmark));
        publishChanged(roomId, userId, RoomBookmarkEventType.BOOKMARK_UPDATED, result.bookmarkId(),
                result.categoryId());
        return result;
    }
    ...
    private void publishChanged(UUID roomId, Long actorUserId, RoomBookmarkEventType type, Long bookmarkId,
                                Long categoryId) {
        eventPublisher.publishEvent(new RoomBookmarkChangedEvent(roomId, actorUserId, type, bookmarkId, categoryId));
    }
}
